...
$PolicyDefinition1 = Get-AzPolicyDefinition | Where-Object { $_.Properties.DisplayName -eq '[Preview]: Deploy requirements to audit Windows VMs that do not have the password complexity setting enabled' }
$PolicyDefinition2 = Get-AzPolicyDefinition | Where-Object { $_.Properties.DisplayName -eq '[Preview]: Audit Windows VMs that do not have the password complexity setting enabled' }
...
foreach ($ResourceGroup in $ResourceGroupObjects) {
    ...
    New-AzPolicyAssignment -Name 'audit-vm-pwdcomplex' -DisplayName '[Preview]: Audit Windows VMs that do not have the password complexity setting enabled' -Scope $ResourceGroup.ResourceID -PolicyDefinition $PolicyDefinition2
    $assignment1 = New-AzPolicyAssignment -Name 'deployext-vm-pwdcomplex' -DisplayName '[Preview]: Deploy requirements to audit Windows VMs that do not have the password complexity setting enabled' -Scope $ResourceGroup.ResourceID -PolicyDefinition $PolicyDefinition1 -Location 'eastus' -AssignIdentity
